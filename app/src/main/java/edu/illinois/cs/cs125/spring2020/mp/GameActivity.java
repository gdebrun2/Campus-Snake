    ...
    private void receivedData(final JsonObject message) {
        ...
        switch (type) {
            case "full":
                ...
                if (message.get("mode").getAsString().equals("area")) {
                    game = new AreaGame(FirebaseAuth.getInstance().getCurrentUser().getEmail(),
                            map, webSocket, message, this);
                } else if (message.get("mode").getAsString().equals("target")) {
                    game = new TargetGame(FirebaseAuth.getInstance().getCurrentUser().getEmail(),
                            map, webSocket, message, this);
                }
                if (game != null) {
